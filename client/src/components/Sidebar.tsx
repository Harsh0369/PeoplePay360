...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, isBackendConnected = false, counts = {} }) => {
  ...
  return (
    <aside className="w-64 bg-brand-850 text-white flex-shrink-0 flex flex-col justify-between border-r border-brand-900/60 z-30 shadow-sidebar select-none">
      <div className="flex flex-col flex-1 overflow-y-auto no-scrollbar">
        ...
        <nav aria-label="Primary" className="p-4 space-y-1">
          ...
          {main.map((i) => <NavLink key={i.tab} item={i} active={activeTab === i.tab} count={counts[i.tab]} onClick={() => onTabChange(i.tab)} />)}
          ...
          {admin.map((i) => <NavLink key={i.tab} item={i} active={activeTab === i.tab} count={counts[i.tab]} onClick={() => onTabChange(i.tab)} />)}

          {me.length > 0 && <div className="pt-4 px-3 py-2 text-[10px] font-bold tracking-wider text-emerald-200/50 uppercase">Personal</div>}
          {me.map((i) => <NavLink key={i.tab} item={i} active={activeTab === i.tab} count={counts[i.tab]} onClick={() => onTabChange(i.tab)} />)}
        </nav>
      </div>
