...
export const ContractList: React.FC<ContractListProps> = ({
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5">
      {/* Header Bar */}
      <div className="bg-brand-offWhite p-4 sm:p-5 rounded-xl border border-brand-sandBorder shadow-sm flex items-center justify-between">
        <div>
          <h2 className="text-base font-extrabold text-brand-darkCharcoal flex items-center">
            <FileText className="w-5 h-5 mr-2 text-brand-darkTeal" />
            Contracts Directory
          </h2>
          <p className="text-xs text-brand-mutedSlate font-medium mt-0.5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs min-w-[750px]">
            <thead className="bg-brand-softSand border-b border-brand-sandBorder text-brand-deepTeal uppercase font-bold text-[11px] tracking-wider">
              <tr>
                <th className="py-3.5 px-5">Contract Ref</th>
            ...
            <tbody className="divide-y divide-brand-sandBorder/60 text-brand-darkCharcoal">
              {contracts.map((cnt) => {
                ...
                return (
                  ...
                  >
                    <td className="py-3.5 px-5 font-mono font-bold text-brand-darkCharcoal">
                      <span className="flex items-center text-brand-darkTeal">
                        <FileText className="w-3.5 h-3.5 mr-1.5 text-brand-mutedSlate flex-shrink-0" />
                        {cnt.contractRef}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 font-extrabold text-brand-darkCharcoal">
                      {cnt.employeeName}
                    </td>
                    <td className="py-3.5 px-4 text-brand-mutedSlate">
                      {cnt.department}
                    </td>
                    <td className="py-3.5 px-4 text-brand-mutedSlate">
                      {cnt.jobPosition}
                    </td>
                    <td className="py-3.5 px-4 text-brand-mutedSlate font-mono text-[11px]">
                      <div className="flex items-center">
                        <Calendar className="w-3.5 h-3.5 mr-1.5 text-brand-mutedSlate flex-shrink-0" />
                        <span>{cnt.startDate} → {cnt.endDate || 'Indefinite'}</span>
                      </div>
                    </td>
